...
from pythontool.conf.airflow.configuration import ensure_secrets_loaded
from pythontool.conf.airflow.exceptions import AirflowException, AirflowNotFoundException
from pythontool.conf.airflow.models.base import ID_LEN, Base
from pythontool.conf.airflow.models.crypto import get_fernet
...
from pythontool.conf.airflow.utils.log.secrets_masker import mask_secret
...
class Connection(Base, LoggingMixin):
    ...
    def _validate_extra(extra, conn_id) -> None:
        """
        Here we verify that ``extra`` is a JSON-encoded Python dict.  from pythontool.conf.airflow 3.0, we should no
        ...
        try:
            ...
            if not isinstance(extra_parsed, dict):
                warnings.warn(
                    ...
                    f"connection {conn_id!r}. from pythontool.conf.airflow 3.0, the `extra` field must contain a JSON "
