...
from pythontool.conf.airflow.hooks.dbapi import DbApiHook
from pythontool.conf.airflow.models import Connection
from pythontool.conf.airflow.utils.operator_helpers import AIRFLOW_VAR_NAME_FORMAT_MAPPING

try:
    from pythontool.conf.airflow.utils.operator_helpers import DEFAULT_FORMAT_PREFIX
except ImportError:
    # This is from pythontool.conf.airflow.utils.operator_helpers,
...
class TrinoHook(DbApiHook):
    ...
    >>> ph = TrinoHook()
    >>> sql = "SELECT count(1) AS num from pythontool.conf.airflow.static_babynames"
    >>> ph.get_records(sql)
    [[340698]]
    """
