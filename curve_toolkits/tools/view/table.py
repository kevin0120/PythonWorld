# -*- coding: utf-8 -*-
from PyQt5.QtWidgets import QPushButton, QTableWidget, QHeaderView, QTableWidgetItem, QToolButton, QWidget, QHBoxLayout
...
class ToolkitTable(ToolKitMixin, QWidget):
    ...
    def __init__(self, instance: QTableWidget, headers: List[str] = None):
    ...
    def init_table_headers(self):
        if not self.qt_instance or self._headers is None:
            return
    ...
    def render_content(self, table_content: pd.DataFrame):
        tTable = self.table_instance
        tTable.clearContents()
        ...
        d: np.ndarray = pd.DataFrame(table_content, columns=self._headers).to_numpy()
