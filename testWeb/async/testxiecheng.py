...
async def func5(i):
    # get the event loop
    ret = await asyncio.to_thread(func01, i)
    return ret
...
if __name__ == "__main__":
    ...
    # #  run_in_executor       ex= futures.ThreadPoolExecutor()
    t1 = loop.time()
    # 将协程转为task，并组成list
    ...
    print(f"得到执行有序序结果 {done}")
    t2 = loop.time()
    print('运行一个异步函数func3 100次，func2中使用run_in_executor，并新建一个线程池ThreadPoolExecutor 运行阻塞代码耗时:', t2 - t1)
    ...
    print(f"得到执行有序序结果 {done}")
    t2 = loop.time()
    print('运行一个异步函数func2 100次，func2中使用run_in_executor 使用默认的线程池，运行阻塞代码耗时:', t2 - t1)
    ...
    # print(f"得到执行有序序结果 {done}")
    # t2 = loop.time()
    # print('运行一个异步函数func4 100次，func2中使用run_in_executor，并新建一个进程池ProcessPoolExecutor 运行阻塞代码耗时:', t2 - t1)
    ...
    # t1 = loop.time()
    # # 将协程转为task，并组成list
    # tasks = [asyncio.ensure_future(func5(i)) for i in range(100)]
    # done = loop.run_until_complete(asyncio.gather(*tasks))
    # print(f"得到执行有序序结果 {done}")
    # t2 = loop.time()
    # print('运行一个异步函数func5 100次，func2中使用to_thread 运行阻塞代码耗时:', t2 - t1)
